 ...
 */
    ...
    class XmlDataWriter extends DataWriter {
        ...
        public function __construct() {
            ...
            $this->_REQ_STR_TPL__ = "<request><auth>".self::__AUTH_PLACEHOLDER__.
                "</auth><data>".self::__DATA_PLACEHOLDER__."</data></request>";
        ...
        private function embedAuthData(){
            $writer = self::initXmlWriter();
            $this->writeAuthData($writer);
            
            // prepare output and do replacement
            $xmlData = $writer->outputMemory();
            $this->_requestBody = str_replace(self::__AUTH_PLACEHOLDER__, $xmlData, $this->_requestBody);
        }
        
        protected function writeAuthData(&$store) {
            if (is_null($store) || $store instanceof \XMLWriter === false)
                throw new \Exception("Invalid object reference for writing request authentication data.");
            ...
            switch ($this->_authModel){
                case AuthModel::PORTAL_PASS:
                    $store->writeElement('model', 'ppass');
                    $store->writeElement('factor', $this->generateAuthFactor());
                    break;
                
                case AuthModel::API_KEY:
                    $store->writeElement('model', 'apikey');
                    $store->writeElement('apiKey', $this->_authApiKey);
                    break;
            ...
            // general
        //    $store->writeElement('pid', $this->_authPID);
            
            if ($this->_authLoadAPS)
                $store->writeElement ('ldas', "true");
        }
        ...
        public function &writeAuthRequest() {
            try {
                $this->embedAuthData();
                 ...
                 // there is nothing for the data placeholder so write empty string
                $this->_requestBody = str_replace(self::__DATA_PLACEHOLDER__, "", $this->_requestBody);
                ...
                throw new \Exception("Could not generate authentication fragment: ".$ex->getMessage());
            }
        }
      ...
      */      
        ...
        private function writeMessageTemplateId($templateId, $xmlWriter){
            $xmlWriter->writeElement('templateId', $templateId);
        }
    }