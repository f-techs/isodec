...
if($_FILES['img_file'] && isset($_POST['collaboration_content']) && isset($_POST['img_caption'])){
    ...
    if(!empty($collaboEntry)){
       ...
       $sql=DB::getInstance()->update('tbl_about', 3 ,'about_type', array('img'=>$imgname, 'img_description'=>$imgcaption, 'details'=>$details, 'modified_by'=>$_SESSION['userID']));
    }else{
        $sql=DB::getInstance()->insert('tbl_about', array('about_type'=>3, 'img'=>$imgname, 'img_description'=>$imgcaption, 'details'=>$details,  'created_by'=>$_SESSION['userID'], 'modified_by'=>$_SESSION['userID']));// todo:use session  
    }
    if(isset($sql)){
        //$response['error']=0;
