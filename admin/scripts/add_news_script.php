...
if(isset($_POST['action'])){
    ...
    if($_POST['action']=='add'){
        $sql=DB::getInstance()->insert('tbl_news', array('news_code'=>$code, 'news_details'=>$details, 'news_title'=>$title, 'news_img'=>$imgname, 'img_description'=>$imgcaption, 'created_by'=>1, 'created_date'=>$date, 'modified_by'=>1, 'modified_date'=>$date));
        if(isset($sql)){
            //$response['error']=0;
        ...
        $ID=$_POST['news_id'];
        $sql=DB::getInstance()->update('tbl_news', $ID, 'news_id',  array('news_details'=>$details, 'news_title'=>$title, 'news_img'=>$imgname, 'img_description'=>$imgcaption, 'modified_by'=>1, 'modified_date'=>$date));
        if(isset($sql)){
        //$response['error']=0;
        $response['status']='success';
