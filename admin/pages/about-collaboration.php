									...
									<div class="card-body">
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<textarea id="collaboration_content" name="collaboration_content" class="form-control" <?php echo (!empty($collaboID)) ? '' : 'required'; ?>><?php echo (!empty($collaboID)) ? $collaboDetails : ''; ?></textarea>
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<div class="image-input image-input-outline" id="img" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/avatars/image.png)">
													<div class="image-input-wrapper" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/uploadImages/<?php echo (!empty($collaboID)) ? $collaboImg : ''; ?>)"></div>

													<label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change image">
														<i class="fa fa-pen icon-sm text-muted"></i>
														<input type="file" name="img_file" id="img_file" accept=".png, .jpg, .jpeg" <?php echo (!empty($collaboID)  && !empty($collaboImg)) ? '' : 'required'; ?> />
														<input type="hidden" id="image_remove_name" value="<?php echo (!empty($collaboID)) ? $collaboImg : ''; ?>" name="profile_avatar_remove" />
													</label>
													<!--
												...
												<!--remove image -->
												<span style="display:<?php echo (!empty($collaboID)  && !empty($collaboImg)) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-danger btn-hover-text-primary btn-shadow" id="img_remove" data-toggle="tooltip" title="Remove image">
													<i class="ki ki-bold-close icon-xs"></i>
												</span>

												<!--view image-->
												<span style="display:<?php echo (!empty($collaboID)  && !empty($collaboImg)) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-success btn-hover-text-primary btn-shadow" id="view_image" data-toggle="tooltip" title="View image">
										...
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<textarea id="img_caption" name="img_caption" class="form-control" <?php echo (!empty($collaboID)) ? '' : 'required'; ?>><?php echo (!empty($collaboID)) ? $collaboImgCaption : ''; ?></textarea>
											</div>
										</div>
									</div>
									...
									<div class="modal-dialog" role="document">
										<div class="modal-content">
											<div class="modal-header">
												<h5 class="modal-title" id="exampleModalLabel">Collaboration Image</h5>
												<button type="button" class="close" data-dismiss="modal" aria-label="Close">
													<i aria-hidden="true" class="ki ki-close"></i>
												</button>
											</div>
											<div class="modal-body">
												<img src="<?php echo URLROOT ?>/assets/admin/media/uploadImages/<?php echo (!empty($collaboID)) ? $collaboImg : ''; ?>" style="width:100%;height:100%; display:block;" />
											</div>
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	...
	$('#img_remove').on('click', function() {
		...
		if (imgname !== '') {
			Swal.fire({
				...
				if (result.value) {
					$.post('<?php echo URLROOT ?>/admin/scripts/about_image_delete_script.php', {
						imgname_collabo: imgname
					}, function(response) {
						if (response === 'success') {
	...
	var CollaborationNote = function() {
		// Private functions
		var collaborationEditor = function() {
			$('#collaboration_content').summernote({
				height: 150
			});
		}
	...
	$('#btn_submit').on('click', function() {
		...
		let collaboration_content = '';
		if ($('#collaboration_content').summernote('isEmpty')) {
			collaboration_content = $('#collaboration_content').val('');
			$('#collaboration_content').prop('require', true);
			Swal.fire("Note!", "Mission Description cannot be empty", "warning");
		} else {
			collaboration_content = $('#collaboration_content').val();
			$('#collaboration_content').prop('require', false);
		}
	})
	...
	$('#frmMission').on('submit', function(e) {
		e.preventDefault();
		//get summernote details before post
		$.ajax({
			method: 'POST',
			url: '<?php echo URLROOT ?>/admin/scripts/add_collaboration_script.php',
