                                    ...
                                    <div class="form-group row" id="service_div">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<select class="form-control" name="event_type" id="event_type" required>
													<option value="<?php echo (!empty($eventType)) ? $eventType : ''; ?>" selected><?php echo (!empty($eventTypeName)) ? $eventTypeName : ''; ?></option>
                                        ...
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-10 col-md-10 col-sm-12">
                                                <textarea required id="event_title" name="event_title" class="form-control"><?php echo (!empty($eventTitle)) ? $eventTitle : ''; ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group row">
											<label class="col-form-label text-right col-lg-2 col-sm-12">Event Date:</label>
											<div class="col-lg-10 col-md-10 col-sm-12">
												<input id="event_date" type="date" value="<?php echo (!empty($eventDate)) ? $eventDate : ''; ?>"   required name="event_date" class="form-control"/>
											</div>
										</div>
										<div class="form-group row">
											<label class="col-form-label text-right col-lg-2 col-sm-12">Event Time:</label>
											<div class="col-lg-10 col-md-10 col-sm-12">
												<input id="event_time" type="time" value="<?php echo (!empty($eventTime)) ? $eventTime : ''; ?>" required name="event_time" class="form-control"/>
											</div>
										</div>
                                        <div class="form-group row">
                                            <label class="col-form-label text-right col-lg-2 col-sm-12">Type Content Here:</label>
                                            <div class="col-lg-10 col-md-10 col-sm-12">
                                                <textarea id="event_content" name="event_content" class="form-control"><?php echo (!empty($eventDetails)) ? $eventDetails : ''; ?></textarea>
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            ...
                                            <div class="col-lg-10 col-md-10 col-sm-12">
                                                <div class="image-input image-input-outline" id="img" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/avatars/image.png)">
                                                    <div class="image-input-wrapper" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/uploadImages/events/<?php echo (!empty($eventImg)) ? $eventImg : ''; ?>)"></div>
                                                    <label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change image">
                                                        <i class="fa fa-pen icon-sm text-muted"></i>
                                                        <div id="file_wrapper">
                                                            <input type="file" name="img_file" id="img_file" accept=".png, .jpg, .jpeg" <?php echo (!empty($eventImg)) ? '' : 'required'; ?> />
                                                        </div>
                                                        <input type="hidden" id="image_remove_name" value="<?php echo (!empty($eventImg)) ? $eventImg : ''; ?>" name="profile_avatar_remove" />
                                                        <input type="hidden" name="event_code" id="event_code" value="<?php echo (!empty($eventCode)) ? $eventCode : ''; ?>" name="profile_avatar_remove" />
                                                    </label>
                                                    <!--
...
-->
                                                ...
                                                <!--remove image -->
                                                <span style="display:<?php echo (!empty($eventImg)) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-danger btn-hover-text-primary btn-shadow" id="img_remove" data-toggle="tooltip" title="Remove image">
                                                    <i class="ki ki-bold-close icon-xs"></i>
                                                </span>
                                                <!--view image-->
                                                <span style="display:<?php echo (!empty($eventImg)) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-success btn-hover-text-primary btn-shadow" id="view_image" data-toggle="tooltip" title="View image">
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <img src="<?php echo URLROOT ?>/assets/admin/media/uploadImages/events/<?php echo (!empty($eventImg)) ? $eventImg : ''; ?>" style="width:100%;height:100%; display:block;" />
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    $('#btn_submit').on('click', function() {
        ...
        let event_content = '';
        if ($('#event_content').summernote('isEmpty')) {
            event_content = $('#event_content').val('');
            $('#event_content').prop('require', true);
            Swal.fire("Note!", "Mission Description cannot be empty", "warning");
        } else {
            event_content = $('#event_content').val();
            $('#event_content').prop('require', false);
        }
    })
    ...
    $('#frmevent').on('submit', function(e) {
        ...
        $.ajax({
            method: 'POST',
            url: '<?php echo URLROOT ?>/admin/scripts/add_event_script.php',
