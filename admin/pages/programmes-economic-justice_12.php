									...
									<div class="card-body">
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<textarea id="mission_content" name="mission_content" class="form-control" <?php echo (!empty($missionID) && $missionID == 1) ? '' : 'required'; ?>><?php echo (!empty($missionID) && $missionID == 1) ? $missionDetails : ''; ?></textarea>
											</div>
										</div>
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<div class="image-input image-input-outline" id="img" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/avatars/image.png)">
													<div class="image-input-wrapper" style="background-image: url(<?php echo URLROOT ?>/assets/admin/media/uploads/<?php echo (!empty($missionID) && ($missionID == 1)) ? $missionImg : ''; ?>)"></div>

													<label class="btn btn-xs btn-icon btn-circle btn-white btn-hover-text-primary btn-shadow" data-action="change" data-toggle="tooltip" title="" data-original-title="Change image">
														<i class="fa fa-pen icon-sm text-muted"></i>
														<div id="file_wrapper">
														<input type="file" name="img_file" id="img_file" accept=".png, .jpg, .jpeg" <?php echo (!empty($missionID)  && !empty($missionImg) && $missionID == 1) ? '' : 'required'; ?> />
														</div>
...
-->
												...
												<!--remove image -->
												<span style="display:<?php echo (!empty($missionID)  && !empty($missionImg) && $missionID == 1) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-danger btn-hover-text-primary btn-shadow" id="img_remove" data-toggle="tooltip" title="Remove image">
													<i class="ki ki-bold-close icon-xs"></i>
												</span>
												<!--view image-->
												<span style="display:<?php echo (!empty($missionID)  && !empty($missionImg) && $missionID == 1) ? 'inline-block' : 'none'; ?>;" class="btn btn-xs btn-icon btn-circle btn-success btn-hover-text-primary btn-shadow" id="view_image" data-toggle="tooltip" title="View image">
										...
										<div class="form-group row">
											...
											<div class="col-lg-10 col-md-10 col-sm-12">
												<textarea id="img_caption" name="img_caption" class="form-control" <?php echo (!empty($missionID) && $missionID == 1) ? '' : 'required'; ?>><?php echo (!empty($missionID) && $missionID == 1) ? $missionImgCaption : ''; ?></textarea>
											</div>
										</div>
									</div>
									...
									<div class="modal-dialog" role="document">
										<div class="modal-content">
											...
											<div class="modal-body">
												<img src="<?php echo URLROOT ?>/assets/admin/media/uploads/<?php echo (!empty($missionID)) ? $missionImg : ''; ?>" style="width:100%;height:100%; display:block;" />
											</div>
										</div>
									</div>
								</div>
...
<script type="text/javascript">
	...
	$('#img_remove').on('click', function() {
		...
		if (imgname !== '') {
			Swal.fire({
				...
				if (result.value) {
					$.post('<?php echo URLROOT ?>/admin/scripts/<?php echo URLROOT ?>/admin/scripts/programme_image_delete_script.php', {
						imgname_mission: imgname
					}, function(response) {
						if (response === 'success') {
	...
	$('#frmMission').on('submit', function(e) {
		...
		$.ajax({
			method: 'POST',
			url: '<?php echo URLROOT ?>/admin/scripts/add_mission_script.php',
